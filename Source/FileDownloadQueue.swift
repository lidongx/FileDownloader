...
public class FileDownloadQueueDelegate {
    ...
    public var onFailed: ((FileDownloadQueue, String, FileDownloadInfo) -> Void)
    public var onProgress: ((FileDownloadQueue, Double) -> Void)
    public init(
        onFinished: @escaping (FileDownloadQueue, [FileDownloadInfo]) -> Void,
        onFailed: @escaping (FileDownloadQueue, String, FileDownloadInfo) -> Void,
...
public class FileDownloadQueue {
    ...
    public convenience init(
        ...
        delegate: FileDownloadQueueDelegate? = nil
    ) {
...
extension FileDownloadQueue {
    ...
    private func createFileDownloadCallback(
        fileInfo: FileDownloadInfo) -> FileDownloadCallback {
        let callback = FileDownloadCallback(fileInfo: fileInfo) { [weak self] _ in
    ...
    private func updateProgress() {
        if downloadItems.count > 0 {
            ...
            progress /= Double(downloadItems.count)
