...
class FileDownloadCallback{
    ...
    init(fileInfo:FileDownloadInfo,onDownloadFinished: @escaping (FileDownload) -> Void, onDownloadFailed: @escaping (FileDownload, String) -> Void, onDownloadProgress: @escaping (FileDownload, Double) -> Void) {
...
class FileDownload: NSObject {
    ...
    func performAfterDelay(interval:Int){
        ...
        let timer = Timer.scheduledTimer(withTimeInterval: TimeInterval(interval), repeats: false) { [weak self]  t in
            DispatchQueue.main.async {
                self?.cancelTimer()
                self?.restart()
            }
